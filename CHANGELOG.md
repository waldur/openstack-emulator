...
### Fixed
- Answer a router deletion the way Neutron does: `RouterNotFound` (404) for one that is not
  there, `RouterInUse` (409) for one that still holds an interface — both in Neutron's own
  error envelope — and delete a router whose only port is its external gateway
