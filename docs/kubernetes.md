...
- **Presets change on upgrade, other config does not.** Changing `customPreset.yaml` and running `helm upgrade` now rolls the pod, because the Deployment carries a `checksum/preset` annotation over the ConfigMap. Note this covers the preset only: the emulator reads a preset once at startup, so anything else you change outside the Deployment spec still needs `kubectl -n ose rollout restart deploy/openstack-emulator`.
