...
def compare_openapi_specs(reference_path: str, emulator_path: str, service_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'error': f"Failed to load specifications: {str(e)}"
        }
    ...
    for endpoint_key in common_endpoints:
        ...
        param_analysis = analyze_parameters(
            ref_endpoint.get('parameters', []),
            emu_endpoint.get('parameters', [])
        )
        
        response_analysis = analyze_responses(
            ref_endpoint.get('responses', {}),
            emu_endpoint.get('responses', {})
        )
        ...
        endpoint_analysis[endpoint_key] = {
            ...
            'operation_id_match': ref_endpoint.get('operation_id') == emu_endpoint.get('operation_id')
        }
    
    # Calculate coverage statistics
    total_endpoints = len(ref_endpoint_keys)
    implemented_endpoints = len(common_endpoints)
    coverage_percentage = (implemented_endpoints / total_endpoints * 100) if total_endpoints > 0 else 0
    ...
    report = {
        ...
        'summary': {
            ...
            'compliance_status': 'good' if coverage_percentage >= 80 else 'partial' if coverage_percentage >= 60 else 'limited'
        },
        'missing_endpoints': {
        ...
        'extra_endpoints': {
            'total': len(extra_endpoints),
            'list': list(extra_endpoints)
        },
        'endpoint_analysis': endpoint_analysis,
        'recommendations': generate_recommendations(missing_endpoints, ref_endpoints, coverage_percentage)
    }
    
    return report


def generate_recommendations(missing_endpoints: Set[str], ref_endpoints: Dict, coverage_percentage: float) -> List[str]:
    """Generate actionable recommendations based on analysis."""
    recommendations = []
    
    if coverage_percentage < 50:
        recommendations.append("Low API coverage detected. Focus on implementing core CRUD operations first.")
    ...
    if len(server_endpoints) > 5:
        recommendations.append("Multiple server management endpoints missing. Consider implementing server lifecycle operations.")
    
    volume_endpoints = [ep for ep in missing_endpoints if 'volume' in ep.lower()]
    if volume_endpoints:
        recommendations.append("Volume management endpoints missing. These are critical for storage operations.")
    
    metadata_endpoints = [ep for ep in missing_endpoints if 'metadata' in ep]
    if metadata_endpoints:
        recommendations.append("Metadata endpoints missing. Consider implementing for better resource management.")
    
    console_endpoints = [ep for ep in missing_endpoints if 'console' in ep]
    if console_endpoints:
        recommendations.append("Console access endpoints missing. Lower priority unless debugging features needed.")
