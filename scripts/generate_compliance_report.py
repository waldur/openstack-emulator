...
def describe_missing_functionality(
    service_name: str, missing_endpoints: List[str]
) -> Dict[str, str]:
    ...
    for endpoint in missing_endpoints:
        ...
        if endpoint in descriptions:
            ...
            for pattern, desc in descriptions.items():
                if (
                    pattern.replace("{id}", "").replace("{provider}", "").replace("/{id}", "")
                    in endpoint
                ):
...
def generate_service_detail_table(service_name: str, report: Dict) -> str:
    ...
    if extra_endpoints:
        ...
        core_endpoints = [
            ep for ep in extra_endpoints if any(op in ep for op in ["POST", "GET", "PUT", "DELETE"])
        ]
        if core_endpoints:
            lines.append(
                f"- **Core Operations**: {len(core_endpoints)} endpoints (CRUD operations)"
            )

        advanced_endpoints = [
            ep
            for ep in extra_endpoints
            if any(adv in ep for adv in ["metadata", "tags", "action", "stats"])
        ]
        if advanced_endpoints:
            lines.append(
                f"- **Advanced Features**: {len(advanced_endpoints)} endpoints (enhanced functionality)"
            )
    ...
    if missing_categories:
        ...
        for category in priority_order:
            if category in missing_categories:
                ...
                if len(endpoints) > 5:
                    lines.append(
                        f"- *...and {len(endpoints) - 5} more {category.lower()} endpoints*"
                    )
        ...
        if "Core Operations" in missing_categories:
            lines.append(
                f"🔴 **High Priority**: {len(missing_categories['Core Operations'])} core endpoints missing"
            )
        if "Advanced Features" in missing_categories:
            lines.append(
                f"🟡 **Medium Priority**: {len(missing_categories['Advanced Features'])} advanced endpoints"
            )
        if "Administrative" in missing_categories:
            lines.append(
                f"⚪ **Low Priority**: {len(missing_categories['Administrative'])} admin endpoints (cloud operator features)"
            )
